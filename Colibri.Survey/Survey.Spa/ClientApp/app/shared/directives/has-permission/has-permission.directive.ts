...
export class HasPermissionDirective extends MatTooltip implements AfterViewInit {
    ...
    ngAfterViewInit() {
        console.log(this.IsDisable ? this.IsDisable : this._isDisable);
        console.log(this.IsHidden ? this.IsHidden : this._isHidden);
    ...
    private updateView() {
        ...
        if (!result) {
            if (this._isHidden) {
                ...
                const test: any = this.getElementByAttribute(this.HAS_PERMISSION_CTRL_ATTRIBUTE, this.element.nativeElement);
                test.disabled = this._isDisable;
            }
        }
    }
    ...
    private getElementByAttribute(attr: any, root: any) {
        if (root.hasAttribute(attr)) {
            return root;
        }
        const children: any = this._permissionCtrl = root.children;
        for (let i = children.length; i--;) {
            this._permissionCtrl = this.getElementByAttribute(attr, children[i]);
